...
let content = fs.readFileSync('src/components/Feed.jsx', 'utf-8');

// 1. Add states
const states_target = "const [postingLostFound, setPostingLostFound] = useState(false);";
const states_replacement = `const [postingLostFound, setPostingLostFound] = useState(false);
...
const handler_target = `    const handleCreateLostFound = async (e) => {`;
const handler_replacement = `    const handleCreateBarter = async (e) => {
...
const cat_target = `        if (lower.includes('#lostfound') || lower.includes('#lost') || lower.includes('#found') ||`;
const cat_replacement = `        if (lower.includes('#barter') || lower.includes('barter') || lower.includes('trade') || lower.includes('exchange')) {
            return 'barter';
        }
...
// 4. Add to categories
const categories_target = "{ id: 'lostfound', label: 'Lost & Found', icon: '🔍' },";
const categories_replacement = `{ id: 'lostfound', label: 'Lost & Found', icon: '🔍' },
        { id: 'barter', label: 'Barter System', icon: '🔄' },`;
content = content.replace(categories_target, categories_replacement);

// 5. Add Banner
const banner_target = "                    {activeCategory === 'lostfound' && (";
const banner_replacement = `                    {activeCategory === 'barter' && (
                        <div 
...
// 6. Add Modal
const modal_target = `            {/* Create Lost & Found Modal */}`;
const modal_replacement = `            {/* Create Barter Modal */}
            {showCreateBarter && (
...
fs.writeFileSync('src/components/Feed.jsx', content, 'utf-8');
console.log("Barter changes injected successfully via Node!");
